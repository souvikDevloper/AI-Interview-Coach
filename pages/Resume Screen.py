...
def init_state(position: str, resume_text: str):
        st.session_state.source_text = resume_text
        st.session_state.setdefault("history", [Message("ai", "Hello! Let’s discuss your resume. Give a brief intro.")])
        ...
        if "retriever" not in st.session_state:
            st.session_state.retriever = _build_retriever(resume_text)
        ...
        if "guideline" not in st.session_state:
            # per-role prompt
            chain_kwargs = prompt_sector(position, templates)
            prompt_tmpl = chain_kwargs.get("prompt", PromptTemplate(input_variables=["context", "question"], template=templates.jd_template))
            query = "Create an interview guideline focused on the uploaded resume. Keep questions short."
            ctx = _retrieve_context(st.session_state.retriever, query)
            prompt = prompt_tmpl.format(context=ctx, question=query)
            try:
        ...
        if "questions" not in st.session_state:
            fallback = [
                ...
                "Tell me about a tough bug you fixed recently.",
                "How does your experience align with this role?",
                "What trade-offs did you make in your biggest project?",
            ]
...
def _handle_input(blob, auto_play: bool):
        ...
        if q_idx < len(st.session_state.questions) and q_idx < MAX_QUESTIONS:
            ...
            next_q = "Thanks — that concludes the resume interview."
...
st.set_page_config(page_title="Resume Screen", page_icon="📄")
st.title("📄 Resume-based Interview")

uploaded = st.file_uploader("Upload your resume (PDF)", type=["pdf"])
position = st.text_input("Target role (e.g., Backend Engineer, ML Engineer, SDE Intern)", "")

if uploaded is not None and position.strip():
        resume_text = _pdf_to_text(uploaded)
        if not resume_text.strip():
            st.warning("Could not extract text from the PDF. Try a text-based PDF (not scanned).")

        init_state(position.strip(), resume_text)
        ...
        if st.session_state.get("report_text"):
            st.download_button("Download report.txt", st.session_state.report_text, file_name="resume_report.txt", mime="text/plain")
        ...
        st.info("Upload a resume PDF and enter a target role to start.")
